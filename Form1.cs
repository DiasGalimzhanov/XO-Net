...
namespace XO
{
    public partial class GameXO : Form
    {
        ...
        private void panel1_Click(object sender, EventArgs e)
        {
            ...
            {
                if((pn.Tag as string)[0] == ' ')
                {
                    string[] temp = (pn.Tag as string).Split(';');
                    MyStep.Y = int.Parse(temp[1]) - 1;
                    MyStep.X = int.Parse(temp[2]) - 1;
        ...
        //
        AutoResetEvent UserStep = new AutoResetEvent(false); // событие хода игрока
        ...
        private void ClientProc(object par)
        {
            ...
            try
            {
                form.tcpClient.Client.BeginReceive(form.buffer, 0, form.buffer.Length, SocketFlags.None, CallbackReseive, form);
        ...
        private void CallbackReseive(IAsyncResult ar)
        {
            if (ar.IsCompleted)
            {
                ...
                try
                {
                    ...
                    Point pt = new Point(int.Parse(temp2[1]), int.Parse(temp2[0]));
                    form.EnemyStep = pt;
